...
class SelectPopup extends Component {
  ...
  fetchApi = debounce((apiUrl, value) => {
    ...
    fetch(`${apiUrl}?name=${value}`)
      ...
      .then((result) => {
        ...
        this.setState({
          items: [...citiesForUserCountry, ...restCities],
          displayItems: [],
          isFetching: false,
        });
      });
  ...
  onClickItem = options => this.props.options.onClose(options);

  blurInput = () => this.input.blur();

  render() {
    const { displayItems, value } = this.state;
    const {
      closePopup,
      options,
    } = this.props;

    const { ItemComponent, title } = options;
    return (
